...
public class CaesarCracker {
    ...
    public final boolean bruteForceDecrypt() {
        //String desencriptar= encryptedMessage;
        int[] key = new int[getPasswordLength()];
        for(int j = 1; j<128; j++){
            key = cargarKey(key,j);
            String s = decode(encryptedMessage,key);
            if(Objects.equals(s, messageWord)){
                foundKey=key;
                return true;
            }
        }
    ...
    private int[] cargarKey (int[] key, int j){
