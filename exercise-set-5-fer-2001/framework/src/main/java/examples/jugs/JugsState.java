...
public class JugsState implements State, Comparable<JugsState> {
  ...
  @Override
  public int compareTo(JugsState o) { // Se valora mejor al elemento más cercano al cero
    if( (Math.abs(this.contentsA) -2) < (Math.abs(o.contentsA) -2)){
      return 1;
    }
    else if((Math.abs(this.contentsA) -2) > (Math.abs(o.contentsA) -2)){
      return -1;
    }
    else{
      return 0;
